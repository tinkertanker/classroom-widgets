...
Launching Classroom Widgets opens a searchable widget launcher. The app also stays available from the menu bar on macOS and the system tray on Windows and Linux for quick access; pick a widget straight from the menu, most used first, with its keyboard shortcut shown beside it. Launch-at-login starts quietly without opening the launcher.
...
The macOS app also includes **Display** for showing an already-connected extended display in a floating window. Open it from the top of the menu-bar menu or its editable keyboard shortcut: when Screen Recording is already granted it immediately shows the remembered display, or the only other eligible one, and the power control requests permission if it is not; with several displays and none remembered, pick the source from its **…** menu. Capture stays local to the Mac and continues while you switch apps; it temporarily suspends if the preview overlaps its source display, and a brief frame gap on the same display holds the last image while it reconnects, with pointer clicks disabled until a fresh frame arrives. The preview viewport always keeps the source display's aspect ratio, fitted to whichever side of a resize is smaller, and its **…** menu re-snaps the same fit on demand with **Match Display Aspect Ratio**; the native minimum size and the on-screen bounds still win if an exact ratio cannot satisfy both. Turn the power control off or close the window to stop it. Clicking its live image moves the pointer to the corresponding point without clicking the destination.
