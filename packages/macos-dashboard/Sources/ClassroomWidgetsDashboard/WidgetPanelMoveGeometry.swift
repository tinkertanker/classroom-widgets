...
/// Pure geometry for the "Move to Next Display" shortcut. Work areas are
/// ordered by origin (x, then y) and wrap around; the panel keeps its size
/// and its offset from the source work area's origin, clamped into the
/// target work area.
enum WidgetPanelMoveGeometry {
    static func nextDisplayFrame(frame: CGRect, workAreas: [CGRect]) -> CGRect? {
        ...
        let target = sorted[(sourceIndex + 1) % sorted.count]
        return CGRect(
