...
const VoiceInterface: React.FC<VoiceInterfaceProps> = ({
  ...
  useEffect(() => {
    ...
    if (transcript && !isGathering && !isListening && voiceState !== 'idle') {
      ...
      if (confidence < 0.5 || transcript.trim().length < 3) {
        debug(`[${new Date().toISOString()}] 🚫 Skipping low-quality transcript: "${transcript}" (confidence: ${confidence})`);
      ...
      debug(`[${new Date().toISOString()}] ✅ Passed quality check for transcript: "${transcript}" (confidence: ${confidence})`);
      ...
      if (transcriptRequestId === processedTranscript || isProcessingRef.current || processedRequestIdsRef.current.has(transcriptRequestId)) {
        debug(`[${new Date().toISOString()}] 🚫 Skipping duplicate transcript processing: "${transcript}"`);
        return;
      }

      const processingId = Math.random().toString(36).slice(2, 11);
      debug(`[${new Date().toISOString()}] [${processingId}] 🎯 Processing transcript: "${transcript}" (confidence: ${confidence})`);
      debug(`[${new Date().toISOString()}] [${processingId}] 📊 State check: isGathering=${isGathering}, isListening=${isListening}, voiceState=${voiceState}, processedTranscript=${processedTranscript}, isProcessingRef=${isProcessingRef.current}`);
      ...
      onTranscriptComplete(transcript)
        .then((response) => {
          debug(`[${new Date().toISOString()}] [${processingId}] ✅ Transcript processing complete:`, response);
          debug(`[${new Date().toISOString()}] [${processingId}] 🔍 Response analysis: feedback.type=${response.feedback.type}, command.action=${response.command.action}`);
          ...
          debug(`[${new Date().toISOString()}] [${processingId}] 🎯 Setting voice state to: ${isError ? 'error' : 'success'}`);
          debug(`[${new Date().toISOString()}] [${processingId}] 📝 Error message: ${response.feedback.message}`);
          setVoiceState(isError ? 'error' : 'success');
          debug(`[${new Date().toISOString()}] [${processingId}] ✅ Voice state set to: ${isError ? 'error' : 'success'}`);
  ...
  useEffect(() => {
    debug(`[${new Date().toISOString()}] 🔄 State transition useEffect: isListening=${isListening}, isGathering=${isGathering}, isProcessing=${isProcessing}, voiceState=${voiceState}, error=${error}`);
    ...
    if ((voiceState === 'error' || voiceState === 'success') && isProcessing) {
      debug(`[${new Date().toISOString()}] 🔄 Keeping current ${voiceState} state - not overriding with processing state`);
      return;
    }

    if (error && voiceState !== 'error') {
      // If there's an error, transition to error state
      debug(`[${new Date().toISOString()}] 🔄 Transitioning to error state due to error: ${error}`);
      setVoiceState('error');
    } else if (isGathering && voiceState !== 'listening') {
      // When gathering starts, transition to listening state
      debug(`[${new Date().toISOString()}] 🔄 Transitioning to listening state`);
      setVoiceState('listening');
    } else if (isProcessing && voiceState !== 'processing') {
      debug(`[${new Date().toISOString()}] 🔄 Transitioning to processing state`);
      setVoiceState('processing');
      ...
      debug(`[${new Date().toISOString()}] 🔄 Transitioning back to activating state`);
      setVoiceState('activating');
    } else {
      debug(`[${new Date().toISOString()}] 🔄 No state transition needed`);
    }
  }, [isListening, isProcessing, isGathering, voiceState, error]);

  const handleClose = () => {
    debug(`[${new Date().toISOString()}] 🔘 Close button clicked`);
    stopRecording();
    resetState();
    setParsedCommand(null);
    ...
    processedRequestIdsRef.current.clear(); // Clear processed request IDs
    setVoiceState('idle');
    debug(`[${new Date().toISOString()}] 🔘 Voice interface closed`);
    onClose();
  };
          ...
          {(voiceState === 'error' || voiceState === 'success') && (
            <div className="flex gap-3 justify-center">
              {voiceState === 'error' && (
                <button
                  onClick={() => {
                    debug(`[${new Date().toISOString()}] 🔘 Retry button clicked`);
                    resetState();
                    setVoiceState('activating');
                    debug(`[${new Date().toISOString()}] 🔘 Reset state and set to activating`);
                    setTimeout(() => {
                      debug(`[${new Date().toISOString()}] 🔘 Starting recording after timeout`);
                      startRecording();
                      setVoiceState('listening');
                    }, 300);
                  }}
