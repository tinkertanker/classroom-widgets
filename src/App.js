...
function App() {
  ...
  useEffect(() => {
    const components = componentList.map(({ id, index }) => {
      ...
      const widgetHeight = index === WIDGET_TYPES.TRAFFIC_LIGHT ? 150 : index === WIDGET_TYPES.RANDOMISER ? 250 : index === WIDGET_TYPES.TEXT_BANNER ? 200 : index === WIDGET_TYPES.SOUND_EFFECTS ? 420 : index === WIDGET_TYPES.TIMER ? 394 : 350;
      ...
      return (
        ...
        key={id}
        id={id}
        lockAspectRatio={index === WIDGET_TYPES.LOUDNESS_MONITOR || index === WIDGET_TYPES.RANDOMISER || index === WIDGET_TYPES.TEXT_BANNER || index === WIDGET_TYPES.IMAGE_DISPLAY || index === WIDGET_TYPES.SOUND_EFFECTS ? false : index === WIDGET_TYPES.TIMER ? (350 / 394) : true}
