...
The application uses a socket-ID-based security model for host authorization.

#### Host Verification

All destructive/admin actions verify the requester is the session host:
...
**Why this is secure:**
- When you create a session, your unique `socket.id` becomes the `hostSocketId`
- Socket IDs are randomly generated by Socket.io and never exposed to students
- An attacker would need to hijack the actual socket connection to impersonate the host

**Protected actions:**
- Create/close rooms
- Update widget state (play/pause)
- Reset polls
- Delete questions
- Clear all data
...
Students must be registered participants to perform actions:

```javascript
if (!session.participants.has(socket.id)) {
  // Reject - not a participant
}
```

### Rate Limiting

Per-event rate limits prevent abuse:

| Event | Limit |
|-------|-------|
| `session:poll:vote` | 2/second |
| `session:linkShare:submit` | 3/5 seconds |
| `session:rtfeedback:submit` | 5/500ms |
| `session:questions:submit` | 2/3 seconds |

Implementation in `server/src/middleware/socketAuth.js`.
...
All socket events validate input server-side using `server/src/utils/validation.js`:
- Session codes: 4-6 alphanumeric characters
- Widget IDs: Non-empty, max 100 chars
- URLs: Valid URL format
- Questions: 1-500 characters
- Feedback values: Integer 1-5
