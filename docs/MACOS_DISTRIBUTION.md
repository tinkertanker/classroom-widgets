...
Display has a single shortcut, defaulting to **Command-Option-Control-0**. It
opens or focuses the single preview window. Clearing the assignment keeps it
unassigned, while menu and launcher actions still open Display. Reopening keeps
the existing macOS source-selection and screen-recording permission behavior.
